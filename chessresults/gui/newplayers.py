...
class NewPlayers(panel.PanedPanelGridSelectorBar):
    ...
    def __init__(self, parent=None, cnf=dict(), **kargs):
        ...
        super(NewPlayers, self).__init__(
            parent=parent,
            cnf=cnf,
            **kargs)
    ...
    def join_merged_players(self):
        ...
        for e in entries:
            h.append(''.join((resultsrecord.get_player_name_text_tabs(
                db, e.value.identity()),)))
        ...
        cdlg = dialogue.ModalConfirm(
            ...
            text='\n\n'.join(('\n'.join(h), '\n'.join(r))),
            action_titles={'Cancel': 'Cancel Merge Player Details',
    ...
    def merge_new_players(self):
        ...
        if len(psel) + len(pbkm) == 0:
            if len(nsel):
                ...
                playerkey = nbkm[0]
            mainentry = mergeplayers.get_new_player_for_alias_key(db, playerkey)
            if mainentry is None:
                dlg = tkinter.messagebox.showinfo(
                    parent=self.get_widget(),
        ...
        a = [''.join((
            resultsrecord.get_player_name_text_tabs(db, e.value.identity()),))
             for e in entries]
        if len(a):
            a.insert(
                0,
