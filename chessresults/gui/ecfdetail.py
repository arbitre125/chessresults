    ...
    def __init__(
        ...
        **kargs):
        ...
        for r, item in enumerate(items):
            st, tc = item
            st_ctrl = tkinter.Label(
                master=self.edit_frame, text=st)
            tc_ctrl = tkinter.Label(
                master=self.edit_frame, text=tc)
...
class ECFClubDialog(ECFDetailDialog):
    ...
    def __init__(self, parent, record, cnf=dict(), **kargs):
        ...
        items = (
            ('Name:', resultsrecord.get_player_name_text(
    ...
    def on_update(self, event=None):
        ...
        if len(ecfname) > 60:
            dlg = tkinter.messagebox.showinfo(
                ...
                message=' '.join((
                    ecfname,
                    'is too long for ECF version of name (max 60)')),
                title='ECF detail Edit')
            return
        if tkinter.messagebox.askyesno(
            parent=self.dialog,
                ...
                if mapperson is not None:
                    dlg = tkinter.messagebox.showinfo(
                        ...
                        message=' '.join((
                            ...
                            'Code for',
                            ''.join((
                                mapperson.value.playername.split('\t', 1)[0],
                                '.')),
