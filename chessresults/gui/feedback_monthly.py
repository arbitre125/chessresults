...
class FeedbackMonthly(panel.PlainPanel):
    ...
    def process_response(self, response):
        ...
        if (fb.feedbacknumbers is None or fb.feedbackplayers is None or
            fb.submissionpins is None or fb.submissionplayers is None):
            message = ''.join((
        ...
        for fbn, spin, sp, fbp in zip(fb.feedbacknumbers,
            ...
            if m:
                ...
                self.insert_text_applyctrl(
                    ''.join(('Pin ', spin.split('=')[1],
                             ': ', n.group(1).strip(), ' : grading code ',
                             m.group(2), ' merged into ',
                             m.group(1), '.\n\n')))
                continue
        self.insert_text_feedbackctrl('\n\n')
        if self.allowapplycodes is None:
            self.insert_text_applyctrl(
                ''.join(('There are no new or merged grading codes ',
                         'to be processed.\n\n')))
        elif not self.allowapplycodes:
            self.insert_text_applyctrl(
                ''.join(('New or merged grading codes will not be processed ',
    ...
    def apply_new_grading_codes(self, *args, **kargs):
        ...
        if (not updateclubs and not updateplayers and
            not newecfcodes and not mergeecfcodes):
            self.insert_text_applyctrl(
