...
class TakeonEdit(panel.PlainPanel):
    ...
    def update_event_results(self):
        ...
        collatedb = takeoncollationdb.TakeonCollationDB(
            self.get_context().get_season().collation,
            db)
    ...
    def show_edits_and_generated(self):
        ...
        if self.editschedctrl == None:
            self.editschedctrl = texttab.make_text_tab(
                master=self.editpane)
        if self.editresctrl == None:
            self.editresctrl = texttab.make_text_tab(
                master=self.editpane)
        ...
        self.schedulectrl.delete('1.0', tkinter.END)
        self.resultsctrl.delete('1.0', tkinter.END)
        self.editschedctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().fixtures, 2))))
        self.editresctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().results, 2))))
        self.schedulectrl.insert(tkinter.END, '\n'.join(
            self.generated_schedule))
    ...
    def show_originals_and_edits(self):
        ...
        self.editresctrl.delete('1.0', tkinter.END)
        self.origschedctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().fixtures, 1))))
        self.editschedctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().fixtures, 2))))
        self.origresctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().results, 1))))
        self.editresctrl.insert(tkinter.END, '\n'.join(
            list(difflib.restore(self.get_context().get_season().results, 2))))
