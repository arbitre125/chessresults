...
class Identities(ExceptionHandler):
    ...
    def identify_new_player(self):
        ...
        if (len(self.lbnew._selection) == 0 and
            len(self.lbknown._selection) == 0):
            tkinter.messagebox.showerror(
                parent=self.get_widget(),
                title='Identify New Players',
                message=' '.join((
                    'Please select a New Player',
                    'and a Known Player.')))
            return
    ...
    def open_import(self):
        ...
        if len(self.format_error):
            ...
            self.lbnew = IdListbox(
                master=fplayers, lblabel='New Players')
            self.lbknown = IdListbox(
                master=fplayers, lblabel='Known Players')
        ...
        for np in newplayers:
            if np not in matchedplayers:
                if np in gameplayermerge:
                    self.local.append((
                        AppSysPersonName(np[0]).name, np, np))
...
class IdListbox(tkinter.Listbox, ExceptionHandler):
    ...
    def _set_select_color(self, event):
        ...
        self._selection = self.index('@' + str(event.x) + ',' + str(event.y)),
...
class MatchListbox(tkinter.Frame, ExceptionHandler):
    ...
    def __init__(self, master, caption, labels):
        ...
        tkinter.Label(
            frame,
            borderwidth=1).pack(fill=tkinter.X)
