...
from ..core import resultsrecord, ecfmaprecord, ecfgcodemaprecord
...
class ResultsDBrowEvent(resultsrecord.ResultsDBrecordEvent, DataRow):
    ...
    def grid_row(self, **kargs):
        ...
        for s in self.value.sections:
            st.append(resultsrecord.get_name_from_record_value(
...
class ResultsDBrowAliasLink(resultsrecord.ResultsDBrecordPlayer, DataRow):
    ...
    header_specification = [
        ...
        {WIDGET: tkinter.Label,
         WIDGET_CONFIGURE: dict(text='Linked Grading Code', anchor=tkinter.W),
         GRID_CONFIGURE: dict(column=4, sticky=tkinter.EW),
...
class ResultsDBrowPlayer(resultsrecord.ResultsDBrecordPlayer, DataRow):
    ...
    def grid_row(self, **kargs):
        ...
        items = (
            self.value.name,
            resultsrecord.get_event_details(self.database, self.value.event),
            resultsrecord.get_section_details(
                self.database,
                self.value.section,
                self.value.pin),
