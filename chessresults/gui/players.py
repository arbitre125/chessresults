...
class Players(panel.PanedPanelGridSelectorBar):
    ...
    def break_one_player_merge(self):
                ...
                for sk in mainentry.value.alias:
                    ...
                    if r is not None:
                        ...
                        if playermap is None:
                            rep.append(resultsrecord.get_player_name_text_tabs(
                                db, rvi))
                        elif pvi != rvi:
                            rep.append(resultsrecord.get_player_name_text_tabs(
                                db, rvi))
        ...
        self.refresh_controls((
            self.playergrid,
            self.aliasgrid,
            (db, filespec.PLAYER_FILE_DEF, filespec.PLAYERPARTIALNEW_FIELD_DEF),
            ))
        ...
        if len(mainentry.value.get_alias_list()):
            ...
            if playermap is None:
                cdlg = dialogue.ModalConfirm(
                    ...
                    action_titles={'Cancel': 'Cancel Demerge Player',
                                   'Ok': 'Demerge Player'},
