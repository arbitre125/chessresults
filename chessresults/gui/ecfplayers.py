...
class ECFPlayers(panel.PanedPanelGridSelectorBar):
    ...
    def __init__(self, parent=None, cnf=dict(), **kargs):
        ...
        super(ECFPlayers, self).__init__(
            parent=parent,
            cnf=cnf,
            **kargs)
    ...
    def break_players_affiliations(self):
        ...
        def build_affiliation_lookup(key):
            ...
            lookup[key] = (
                ...
                resultsrecord.get_player_name_text_tabs(
                    db,
                    resultsrecord.get_alias(db, key).value.identity()))
        ...
        dlg = dialogue.ModalConfirm(
            ...
            text='\n\n'.join(('\n'.join(header), '\n'.join(detail))),
            action_titles={'Cancel': 'Cancel Remove Affiliation Details',
    ...
    def edit_player_ecf_name(self):
        ...
        mr = ecfmaprecord.get_person_for_alias(
            db, resultsrecord.get_person_from_player(
                db, resultsrecord.get_alias(
                    db, psel[0][-1])).key.recno)
        if mr is None:
            pr = resultsrecord.get_alias(db, psel[0][-1])
            dlg = tkinter.messagebox.showinfo(
                parent=self.get_widget(),
                message=''.join((
                    resultsrecord.get_player_name_text(db, pr.value.identity()),
                    '\nrecord has been deleted.\nCannot ',
        ...
        self.refresh_controls((
            self.playerclubgrid,
            (db, filespec.MAPECFPLAYER_FILE_DEF, filespec.PERSONMAP_FIELD_DEF),
            ))
        return
    ...
    def remove_grading_code(self):
        ...
        mr = ecfmaprecord.get_person_for_alias(
            ...
            resultsrecord.get_person_from_player(
                db, resultsrecord.get_alias(db, psel[0][-1])).key.recno)
        if mr is None:
            pr = resultsrecord.get_alias(db, psel[0][-1])
            dlg = tkinter.messagebox.showinfo(
                parent=self.get_widget(),
