    ...
    def __init__(
        ...
        rv = resultsrecord.get_event_from_record_value(
            database.get_primary_record(
                filespec.EVENT_FILE_DEF, event[-1])).value
        ...
        eventaliases = resultsrecord.get_aliases_for_games(database, eventgames)
                ...
                self.summary.append(
                    ''.join(
                        (eventaliases[gv.awayplayer].value.name,
                         '\t\t\t',
                         DISPLAYRESULT[INVERT_RESULT.get(gv.result, gv.result)],
                         '\t',
