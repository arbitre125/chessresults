...
EVENT_SUMMARY_HEADER = (
    ('Event', 'Start Date', 'End Date', 'Section', 'Home Team', 'Away Team',
     'Round', 'Date Played', 'Board',
     'ECF Name Home Player', 'ECF Name Away Player',
     'Home Player', 'Away Player',
     'Result', 'Home Player Colour',
     'Home Person Number', 'Away Person Number', 'Event Number',),
    ('Grading Code', 'ECF Name', 'Player', 'Game Count',
...
class Events(panel.PanelGridSelector):
    ...
    def generate_event_export(self, database, logwidget):
        ...
        for e in export_events:
            rv = resultsrecord.get_event_from_record_value(
                database.get_primary_record(
                    filespec.EVENT_FILE_DEF, e[-1])).value
    ...
    def on_export_events(self, event=None):
        ...
        self.get_appsys().set_kwargs_for_next_tabclass_call(
            dict(
                runmethod=self.get_appsys().get_results_database(
                    ).do_database_task,
                starttaskmsg='Export events task started',
                tabtitle='Export Events',
                runmethodargs=dict(
                    taskmethod=self.generate_event_export),
                taskbuttons={
    ...
    def on_game_summary(self, event=None):
        ...
        self.get_appsys().set_kwargs_for_next_tabclass_call(
            dict(
                ...
                runmethodargs=dict(
                    taskmethod=self.display_game_summary),
                taskbuttons={
                    TaskPanel._btn_closebackgroundtask: dict(
                        text='Cancel',
    ...
    def on_event_summary(self, event=None):
        ...
        self.get_appsys().set_kwargs_for_next_tabclass_call(
            dict(
                ...
                runmethodargs=dict(
                    taskmethod=self.generate_event_summary),
                taskbuttons={
                    TaskPanel._btn_closebackgroundtask: dict(
                        text='Cancel',
    ...
    def _on_save_exported_events(self, event=None):
        ...
        tkinter.messagebox.showinfo(
            ...
            title='Export Event Results',
            message='\n'.join((
                'Event Results saved for export in', filename)))
    ...
    def delete_events(self):
        ...
        if identity_report:
            ...
            dialogue.Report(
                ...
                tabstyle='tabular').append('\n\n'.join(
                    (head,
                     '\n'.join([resultsrecord.get_player_name_text_tabs(
                         db,
                         i.value.identity()) for i in identity_report]),
                     )))
            return
        else:
            dlg = dialogue.ModalConfirm(
                ...
                action_titles={'Cancel': 'Cancel Delete Events',
                               'Ok': 'Delete Events'},
        ...
        def unset_player(skey):
            ...
            if skey not in players:
                players[skey] = resultsrecord.get_alias(
                    db,
                    skey)
    ...
    def calculate_player_performances(self, database, logwidget):
        ...
        for e in calculate_events:
            rv = resultsrecord.get_event_from_record_value(
                database.get_primary_record(filespec.EVENT_FILE_DEF, e[-1])).value
            er = [rv.startdate, rv.enddate, rv.name]
            er.extend(
                [resultsrecord.get_name_from_record_value(
                    database.get_primary_record(
        ...
        if logwidget:
            logwidget.append_text(
                'Calculating player performances.')
            logwidget.append_text_only('')
        names = gefpc[-1]
        for k in names.keys():
            aspn = AppSysPersonName(names[k])
    ...
    def display_game_summary(self, database, logwidget):
        ...
        if logwidget:
            logwidget.append_text(
                'Extracting game summaries for each event.')
            logwidget.append_text_only('')
        ...
        if self.get_appsys().show_master_list_grading_codes:
            ...
            ecfplayers = {
                p:ecfogdrecord.get_ecf_ogd_player_for_grading_code(
                    database,
                    gradingcodes[p]).value.ECFOGDname
                if gradingcodes[p] else ''
                for p in gradingcodes
                }
        else:
            ecfplayers = {}
        ...
        games = sorted(
            [(events[g.value.event].name,
              ...
              ecfplayers[g.value.homeplayer
                         ] if g.value.homeplayer in ecfplayers else '',
              ecfplayers[g.value.awayplayer
                         ] if g.value.awayplayer in ecfplayers else '',
