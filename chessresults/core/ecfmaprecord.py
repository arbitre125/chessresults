...
class ECFmapDBrecordEvent(Record):
    ...
    def __init__(self,
                 keyclass=ECFmapDBkeyEvent,
                 valueclass=ECFmapDBvalueEvent):

        super(ECFmapDBrecordEvent, self).__init__(
            keyclass,
            valueclass)
...
def get_player_for_alias(database, aliaskey):
    ...
    if r is not None:
        if database.encode_record_selector(r[0]) == ak:
            p = database.get_primary_record(filespec.MAPECFCLUB_FILE_DEF, r[-1])
            if p is not None:
