...
class Collation(GameCollation):
    ...
    def _collate_allplayall(self, section):
        ...
        for g in games:
            ...
            sectiongames.games.append(SwissGame(
                tagger=g[7],
                ...
                awayplayer=ap)) #should be the Player instance now
            ...
            for p in g.homeplayer, g.awayplayer,:
                if p.event is None or p.startdate is None or p.enddate is None:
                    ...
                    p.__dict__['_identity'] = (
                        p.name, p.event, p.startdate, p.enddate)
            if g.date is None:
                        ...
                        for umkey in self.matches[mrg.section]:
                            ...
                            for game in match.games:
                                if game.result != tobereported:
                                    continue
                                if (game.homeplayer != mrg.homeplayer or
                                    game.awayplayer != mrg.awayplayer):
                                    continue
    ...
    def collate_matches(self, reports, schedule):
        ...
        for e, f, match in sorted([(m.order, m.source, m) for m in matchrecords]):
            ...
            for u in sorted(unique_match[s]):
                ...
                for pmr in umsu[:-1]:
                    ...
                    for mrmg, prg in zip(mrm.games, pmr.games):
                        ...
                        if problems:
                            match_problems.setdefault(
                                mrmg, set()).update(problems)
                ...
                if not match_problems:
                    ...
                    mp = {k:v for k, v in match_problems.items()
    ...
    def get_players_by_club(self, separator=None):
        ...
        for club in self.clubplayers:
            ...
            for player in clubplayers:
                if player != None:
                    ps.append(
                        (AppSysPersonName(player[0]).name,
                         player))
            ps.sort()
        ...
        for section in self.matches:
            for umkey in self.matches[section]:
                for match in self.matches[section][umkey]:
                    ...
                    if key not in tm:
                        tm[key] = [(match.hometeam,
                                    match.awayteam,
                                    (match, ufg, c))]
                    else:
                        tm[key].append((match.hometeam,
                                        match.awayteam,
                                        (match, ufg, c)))
        return tm
...
class _MatchAuthorization(object):
    ...
    def authorize_match_report(self, match):
        ...
        try:
            ...
            return
        self._dates_ok = (self._authorization_time - max_date >
                          a.authorization_delay)
