...
class Season(object):
    ...
    def open_documents(self, parent):
        ...
        if not os.path.exists(os.path.join(self.folder, EXTRACTED)):
            if os.path.isdir(os.path.join(self.folder, COLLECTED)):
                try:
                    ...
                    pass
        ef = len([f for f in folders
                  if os.path.exists(
                      os.path.join(self.folder, emc.criteria[f]))])
        ...
        for em in emc.selected_emails:
            ...
            email_headers[os.path.splitext(
                em.filename)[0]] = _Headers(
                    dates=em.dates,
        ...
        for p in sorted(os.listdir(folder)):
            ...
            try:
                diff_text.append(
                    _DifferenceText(
                        fp, f.readlines(), headers=email_headers))
...
class _DifferenceText(object):
    ...
    def set_tags(self, suffix):
        """"""
        if (self._data_tag is None and
            self._header_tag is None and
            self._trailer_tag is None):
    ...
    def sender_and_date(self):
        ...
        try:
            if (self._filename[:14].isdigit() and
                self._filename[-4:].isdigit() and
                self._filename[-5] in '+-'):
...
def create_event_configuration_file(file_path):
    ...
    try:
        ...
        cf.writelines(
            (' '.join((EXTRACTED, EXTRACTED)), os.linesep))
        cf.writelines(
            (' '.join((TEXTENTRY, TEXTENTRY)), os.linesep))
        cf.writelines(
    ...
    if COLLECTED + '.conf' not in names:
        ...
        for fn in ems:
            ...
            if e == '.ems' and fn != conflictname:
                ...
                try:
                    if len(ems) > 1:
                        cf = open(os.path.join(directory, n + '.conf'),
                                  'w',
                                  encoding='utf8')
                    else:
                        cf = open(os.path.join(directory, COLLECTED + '.conf'),
                                  'w',
                                  encoding='utf8')
                    try:
                        for line in ocf:
                            m = re.match(
                                r'([\s#]*)(outputdirectory)(\s+)(.*)',
                                line)
                            if m:
